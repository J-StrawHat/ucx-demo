...
void zcopy_completion_cb(uct_completion_t *self)
{
    ...
    desc_holder = (void *)0xDEADBEEF; // 用于标记是否完成
}
                         ...
                         const cmd_args_t *cmd_args, char *buf)
{
    ...
    uct_mem_h memh;
    uct_iov_t iov; //单个数据传输函数调用中使用多个数据缓冲区
    zcopy_comp_t comp;
    ...
    iov.memh   = memh; //帮助远程节点识别和访问本地内存的本地内存键描述符
    iov.stride = 0; //缓冲区中连续有效载荷元素之间的步长
    iov.count  = 1; //缓冲区中有效载荷元素的数量

    comp.uct_comp.func   = zcopy_completion_cb; // 用户定义的回调函数
    comp.uct_comp.count  = 1;                   // 表示还需要多少次操作完成后，才能触发回调函数。每操作一次，该值减1
    comp.uct_comp.status = UCS_OK;              // 完成的状态
    comp.md              = if_info->md;
    comp.memh            = memh;
    ...
    if (status == UCS_OK) {
        do {
            status = uct_ep_am_zcopy(ep, id, NULL, 0, &iov, 1, 0,
                                     (uct_completion_t *)&comp); // 零拷贝发送消息，并且iov中的元素按序处理
            uct_worker_progress(if_info->worker); // 显式的推进通信操作，函数返回一个无符号整数，表示是否有任何通信操作得到了推进。
        } while (status == UCS_ERR_NO_RESOURCE); // 如果没有资源，则需要再次调用零拷贝发送函数

        if (status == UCS_INPROGRESS) {
            while (!desc_holder) {
                /* Explicitly progress outstanding active message request */
                uct_worker_progress(if_info->worker); // 不断地推进通信操作，直到完成
            }
            status = UCS_OK;
        }
    }
                                ...
                                unsigned flags) // data 即为传输的数据
{
    ...
    if (flags & UCT_CB_PARAM_FLAG_DESC) { // 当该标志被设置时，表示回调函数中的数据是描述符的一部分。这个描述符还包含了用户定义的 rx_headroom（接收头部空间）
        rdesc = (recv_desc_t *)data - 1;
        /* Hold descriptor to release later and return UCS_INPROGRESS */
        rdesc->is_uct_desc = 1; // 对描述符进行标记，用于在主函数中进行释放
        desc_holder = rdesc; //将描述符存储在全局变量中
        return UCS_INPROGRESS;
    }
    // 如果 flags 不包含 UCT_CB_PARAM_FLAG_DESC，则表示 data 不能直接在回调函数外使用，需要我们手动拷贝
...
int main(int argc, char **argv)
{
    ...
    status = uct_iface_set_am_handler(if_info.iface, id, hello_world,
                                      &cmd_args.func_am_type, 0); // 指定消息接收后的回调处理函数
    ...
    if (cmd_args.server_name) { // 客户端
        ...
        if (cmd_args.func_am_type == FUNC_AM_SHORT) {
            ...
            status = do_am_zcopy(&if_info, ep, id, &cmd_args, str); // 执行零拷贝发送函数
        }
